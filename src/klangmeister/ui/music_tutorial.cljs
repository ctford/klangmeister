...
(def steps
  ...
  (times 2))"]
   :bpm
   ["So far every unit of time has been a second. If we want to speed up or slow down the music, we can define how many
  ...
  (tempo (bpm 90))) ; Try making the music faster or slower."]
   :key
   ["We have to be careful which pitches we choose. A given piece of music focuses on certain pitches and ignores others. For example, 72 and 74 are in C major, but 73 is not. A convenient way to write songs in-key is to number the notes within the key and then later use code to translate those numbers into specific pitches. Here we use the where function to put each note in C major."
    "(->>
  (phrase [3/3 3/3 2/3 1/3 3/3]
          [  0   0   0   1   2])
  (with

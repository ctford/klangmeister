...
(defn bell! [{:keys [time duration pitch]}]
  (let [harmonic (fn [n proportion]
                   (doto (sin-osc (* n pitch))
                     (from time (+ time 1.5))
                     (connect (perc time 0.01 (* 0.05 proportion) proportion)
                              (.-destination context))))]
...
(defn fuzz! [{:keys [time duration pitch]}]
  (let [envelope (perc time 0.1 0.8 0.5)]
    (doto (saw pitch)
      (from time (+ time 1.5))
...
(defn buzz! [{:keys [time duration pitch]}]
  (let [freqs [pitch (* pitch 1.01) (* pitch 0.99)]
        envelopes [(perc time 0.3 0.4 0.2)
                   (perc time 0.05 0.3 0.1)
                   (perc time 0.1 0.01 0.1)]]
